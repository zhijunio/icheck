...
function RectificationEditor({ item }: { item: RectificationItem }) {
  ...
  async function selectPhoto(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const photo = await fileToDataUrl(file).then(compressImage);
      setRectification(item.itemKey, { rectificationPhotoName: file.name, rectificationPhoto: photo, rectificationVerification: undefined });
    } catch {
  ...
  async function completeRectification() {
    if (!item.photo || !item.originalPhotos.length || !note.trim() || photoUploading || verifying) return;
    const originalPhotos = item.originalPhotos;
    const rectificationPhoto = item.photo;
    setVerifying(true);
    setVerificationError("");
    try {
      const aiOriginalPhotos = await Promise.all(originalPhotos.map(preparePhotoForAi));
      const aiRectificationPhoto = await preparePhotoForAi(rectificationPhoto);
      const response = await fetch("/api/rectification/verify", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ item: item.item, reason: item.reason, acceptanceCriteria: item.acceptanceCriteria ?? "巡检项目要求已达到，整改问题不再存在。", originalPhotos: aiOriginalPhotos, rectificationPhoto: aiRectificationPhoto }),
      });
  ...
  return <div className="mt-4 border-t border-slate-100 pt-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-medium text-slate-800">处理整改</p><p className="mt-1 text-xs text-slate-400">完成整改时自动进行 AI 对比验收并保存记录</p></div><span className={`rounded-full px-3 py-1.5 text-xs font-medium ${status === "已完成" ? "bg-teal-50 text-teal-700" : status === "整改中" ? "bg-sky-50 text-sky-700" : "bg-amber-50 text-amber-700"}`}>{status}</span></div><EvidenceCompare originalPhotos={item.originalPhotos} rectificationPhoto={item.photo} />{status === "待整改" && <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-100 bg-amber-50/60 p-4"><p className="text-sm text-amber-800">确认开始后，才能上传整改后的复核图片。</p><button className="rounded-lg bg-slate-900 px-4 py-2.5 text-sm font-medium text-white hover:bg-slate-700" onClick={() => setRectification(item.itemKey, { rectificationStatus: "整改中" })} type="button">开始整改</button></div>}{status === "整改中" && <div className="mt-4 rounded-xl border border-sky-100 bg-sky-50/50 p-4"><textarea className="min-h-20 w-full resize-y rounded-xl border border-slate-200 bg-white p-3 text-sm text-slate-700 outline-none focus:border-teal-500 focus:ring-4 focus:ring-teal-50" onChange={(event) => setRectification(item.itemKey, { rectificationNote: event.target.value })} placeholder="填写本项目整改说明" value={note} /><div className="mt-3 flex flex-wrap items-center gap-3"><label className={`rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-600 ${photoUploading ? "cursor-wait opacity-60" : "cursor-pointer hover:bg-slate-50"}`}><span>{photoUploading ? "正在读取图片…" : item.photo ? "重新上传整改图片" : "上传整改图片"}</span><input accept="image/*" className="hidden" disabled={photoUploading} onChange={selectPhoto} type="file" /></label>{photoName && <span className="max-w-full truncate text-xs text-slate-500">已选择：{photoName}</span>}{photoError && <span className="text-xs text-rose-600">{photoError}</span>}</div><div className="mt-4 flex justify-end"><button className="rounded-lg bg-teal-700 px-4 py-2.5 text-sm font-medium text-white hover:bg-teal-800 disabled:cursor-not-allowed disabled:opacity-50" disabled={!note.trim() || !photoName || !item.photo || !item.originalPhotos.length || photoUploading || verifying} onClick={completeRectification} type="button">{verifying ? "AI 对比并提交中…" : "提交整改完成"}</button></div>{item.originalPhotos.length === 0 && <p className="mt-3 text-xs text-amber-700">缺少整改前现场图片，无法在完成整改时进行 AI 对比验收。</p>}{verificationError && <p className="mt-3 text-xs text-rose-600">{verificationError}</p>}</div>}{status === "已完成" && <div className="mt-4 rounded-xl border border-teal-100 bg-teal-50/60 px-4 py-3 text-sm text-teal-800">整改已完成，以下保留整改前后图片和 AI 对比验收记录，供后续复核。</div>}{item.verification && <VerificationResult verification={item.verification} />}</div>;
}

function EvidenceCompare({ originalPhotos, rectificationPhoto }: { originalPhotos: string[]; rectificationPhoto?: string }) {
  return <div className="mt-4 grid gap-3 sm:grid-cols-2"><div className="rounded-xl border border-rose-100 bg-rose-50/40 p-3"><div className="flex items-center justify-between gap-2"><p className="text-xs font-semibold text-rose-700">整改前</p><span className="text-[11px] text-rose-500">{originalPhotos.length} 张</span></div>{originalPhotos.length > 0 ? <div className="mt-3 flex flex-wrap gap-2">{originalPhotos.map((photo, index) => <EvidenceThumbnail alt={`整改前现场图片 ${index + 1}`} borderClass="border-rose-100" key={`${photo}-${index}`} src={photo} />)}</div> : <div className="mt-3 flex min-h-20 items-center justify-center rounded-lg border border-dashed border-rose-200 bg-white/70 px-3 text-center text-xs text-rose-500">暂无整改前图片，无法进行 AI 对比</div>}</div><div className="rounded-xl border border-teal-100 bg-teal-50/40 p-3"><div className="flex items-center justify-between gap-2"><p className="text-xs font-semibold text-teal-700">整改后</p><span className="text-[11px] text-teal-500">复核图片</span></div>{rectificationPhoto ? <div className="mt-3"><EvidenceThumbnail alt="整改后复核图片" borderClass="border-teal-100" src={rectificationPhoto} /></div> : <div className="mt-3 flex min-h-20 items-center justify-center rounded-lg border border-dashed border-teal-200 bg-white/70 px-3 text-center text-xs text-teal-600">开始整改后上传复核图片</div>}</div></div>;
}

function EvidenceThumbnail({ alt, borderClass, src }: { alt: string; borderClass: string; src: string }) {
  return <div className={`relative size-20 shrink-0 overflow-hidden rounded-lg border ${borderClass}`}><Image alt={alt} className="object-cover" fill sizes="80px" src={src} unoptimized /></div>;
}
...
function VerificationResult({ verification }: { verification: NonNullable<RectificationItem["verification"]> }) {
  ...
  return <div className={`mt-4 rounded-xl border p-4 ${config.className}`}><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-semibold">整改前后 AI 对比验收</p><p className="mt-1 text-[11px] opacity-70">复核时间：{formatShanghaiTime(verification.verifiedAt)}</p></div><span className="rounded-full bg-white/70 px-2.5 py-1 text-xs font-medium">{config.label}</span></div><p className="mt-3 text-sm leading-6">{verification.summary}</p><p className="mt-2 text-xs leading-5">下一步建议：{verification.recommendation}</p></div>;
}
